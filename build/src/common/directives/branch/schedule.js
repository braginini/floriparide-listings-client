System.registerModule("src/common/directives/branch/schedule.js", [], function() {
  "use strict";
  var __moduleName = "src/common/directives/branch/schedule.js";
  ...
  var $__default = angular.module('directives.branchSchedule', ['ui.bootstrap.tooltip', 'template/tooltip/tooltip-popup.html', 'template/tooltip/tooltip-html-unsafe-popup.html']).directive('branchSchedule', ["$interval", "$timeout", "$parse", "locale", function($interval, $timeout, $parse, locale) {
    return {
      restrict: 'EA',
      ...
      controller: ["$scope", "$attrs", "$element", function($scope, $attrs, $element) {
        ...
        var schedule = _.chain($scope.schedule).values().sortBy(function(item) {
  ...
  }]).directive('scheduleTooltipPopup', ["$window", function($window) {
    return {
      restrict: 'EA',
  ...
  }).filter('formatSchedule', ["locale", function(locale) {
    return formatSchedule(locale);
  }]).filter('formatToday', ["locale", function(locale) {
    return function(item) {
      var res = item.label + ': ';
      res += formatSchedule(locale)(item);
      if (item.breaks && item.breaks.length) {
        var joiner = item.breaks.length === 1 ? ', ' : ', <br>';
...
//# sourceURL=src/common/directives/branch/schedule.js
