var ModalInstanceCtrl = function ($scope, $uibModalInstance, $q, $timeout, $interval, $window, Fullscreen, images, index) {
  ...
  $scope.images = _.map(images, item => {
    var d = $q.defer();
    if (_.isString(item)) {
      return {
        src: item,
  ...
  var syncSizeNow = function () {
    ...
    var w = $scope.current.width, h = $scope.current.height;
    if (!w || !h) {
      return;
    }
    ...
    // we will fit image to dialog width if a little bit smaller to improve ui
    if (dlgWidth - w < 10) {
      w = dlgWidth;
    }
    dlgWidth |= 0;
  ...
  var buildPreloads = function () {
    var i, heights = {}, mostFreqHeight, max = 0;
    $scope.preloads = [];
    $scope.preferredHeight = 0;

    for (i = 0; i < $scope.images.length; i++) {
      let img = $scope.images[i];
  ...
  var initPromise = $interval(function () {
    ...
    if (dlgEl[0]) {
      ...
      $scope.images[index].deferred.promise.then(() => {
        syncSizeNow();
        $scope.readyUI = true;
      });
    }
